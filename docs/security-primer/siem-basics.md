...
!!! info "How Seerflow Uses This"
    ...
    - **Collect:** Seerflow ingests logs through multiple receiver types: **OpenTelemetry** (gRPC and HTTP) for cloud platforms like AWS CloudWatch, GCP Cloud Logging, and Azure Monitor; a native **syslog** receiver (UDP/TCP) for network devices and Linux hosts; a **file tail** receiver for local log files; and a generic **webhook** receiver for any source that can POST JSON. You can run one receiver or all of them at once --- Seerflow normalizes every log into the same internal event format regardless of how it arrived.
    - **Detect:** Seerflow layers multiple detection strategies that work together in real time. **Sigma rules** (a portable, open-source format for writing detection logic) catch known attack patterns with precision --- 63 rules ship bundled and load automatically. **Streaming ML models** (Half-Space Trees, Holt-Winters, Markov chains) learn normal behavior and flag anomalies that no rule has been written for. On top of both, **entity-graph correlation** connects related events across users, IPs, and hosts to detect multi-step attack campaigns that no single event would reveal.
    - **Alert:** When detection fires, Seerflow sends notifications through configurable **webhooks** for Slack, Microsoft Teams, or any HTTP endpoint. For teams that use incident management, Seerflow also integrates with **PagerDuty** to automatically create and resolve incidents based on alert severity.
